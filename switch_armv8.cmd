...
echo =====[ Building V8 ]=====
if "%VERSION%"=="11.8.172" (
    call gn gen out.gn\arm64.release -args="target_os=""win"" target_cpu=""arm64"" v8_use_external_startup_data=false v8_enable_i18n_support=false is_debug=false v8_static_library=true %CXX_SETTING% strip_debug_info=true symbol_level=0 v8_enable_pointer_compression=false v8_enable_sandbox=false v8_enable_maglev=false v8_enable_webassembly=false"
)

if "%VERSION%"=="10.6.194" (
    call gn gen out.gn\arm64.release -args="target_os=""win"" target_cpu=""arm64"" v8_use_external_startup_data=false v8_enable_i18n_support=false is_debug=false v8_static_library=true %CXX_SETTING% strip_debug_info=true symbol_level=0 v8_enable_pointer_compression=false v8_enable_sandbox=false"
)

if "%VERSION%"=="9.4.146.24" (
    call gn gen out.gn\arm64.release -args="target_os=""ns"" target_cpu=""arm64"" v8_use_external_startup_data=false v8_enable_i18n_support=false is_debug=false v8_static_library=true %CXX_SETTING% strip_debug_info=true symbol_level=0 v8_enable_pointer_compression=false"
)
call ninja -C out.gn\arm64.release -t clean
call ninja -v -C out.gn\arm64.release wee8

echo =====[ Building V8 success ]=====
md output\v8\Lib\Win64
if "%NEW_WRAP%"=="with_new_wrap" (
  call %~dp0\rename_symbols_win.cmd arm64 output\v8\Lib\Win64\
)
copy /Y out.gn\arm64.release\obj\wee8.lib output\v8\Lib\Win64\

echo =====[ Copy V8 header ]=====
:xcopy include output\v8\Inc\  /s/h/e/k/f/c

:md output\v8\Bin\Win64
:copy /Y out.gn\arm64.release\v8cc.exe output\v8\Bin\Win64\
:copy /Y out.gn\arm64.release\mksnapshot.exe output\v8\Bin\Win64\
